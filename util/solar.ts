...
const toJulian = (date: Date): number =>
    date.valueOf() / dayMills - 0.5 + julian1970;
...
const getApproxSolarTransit = (
    ht: number,
    lw: number,
    julianCycle: number
): number => julian2000 + julian0 + (ht + lw) / (2 * PI) + julianCycle;
const getSunriseJulianDate = (
    julianTransit: number,
    julianSet: number
): number => julianTransit - (julianSet - julianTransit);
const getSolarTransit = (
    approxSolarTransit: number,
...
const getSunsetJulianDate = (
    ...
    getSolarTransit(
        getApproxSolarTransit(w0, lw, julianCycle),
        meanAnomaly,
        eclipticLongitude
    );
const getHourAngle = (hour: number, phi: number, declination: number): number =>
    acos(
        (sin(hour) - sin(phi) * sin(declination)) /
...
function getMoonCoords(date: Date, days: number): MoonCoordinates {
    ...
    const longitude =
        360 * rp + 6.3 * sin(dp) + 1.3 * sin(2 * ip - dp) + 0.7 * sin(2 * ip);
    const zodiacSign = Object.keys(zodiacs)
        .map((key) => zodiacs[key])
        .filter(
            (zodiac) => longitude >= zodiac.long && longitude < zodiac.long + 30
        )[0].sign;
...
function getSunTimes(date: Date, lat: number, lon: number): SunTimes {
    ...
    const julianSet = getSunsetJulianDate(
        w0,
        solarMeanAnomaly,
        eclipticLongitude,
        lw,
    ...
    const julianSetStart = getSunsetJulianDate(
        w1,
        solarMeanAnomaly,
        eclipticLongitude,
        lw,
    ...
    const julianNautical = getSunsetJulianDate(
        w2,
        solarMeanAnomaly,
        eclipticLongitude,
        lw,
    ...
    const julianAstro = getSunsetJulianDate(
        w3,
        solarMeanAnomaly,
        eclipticLongitude,
        lw,
    ...
    const julianDark = getSunsetJulianDate(
        w4,
        solarMeanAnomaly,
        eclipticLongitude,
        lw,
...
function getMoonPosition(date: Date, lat: number, lon: number): MoonPosition {
    ...
    const parallacticAngle = atan2(
        sin(siderealTime),
        tan(phi) * cos(moonCoords.declination) -
...
function getMoonPhase(date: Date): MoonPhase {
    ...
    const phi = acos(
        sin(sunCoords.declination) * sin(moonCoords.declination) +
            cos(sunCoords.declination) *
                cos(moonCoords.declination) *
                cos(sunCoords.rightAscension - moonCoords.rightAscension)
    );
    const inc = atan2(
        sunDistance * sin(phi),
        moonCoords.distance - sunDistance * cos(phi)
    );
    const angle = atan2(
        cos(sunCoords.declination) *
            sin(sunCoords.rightAscension - moonCoords.rightAscension),
        sin(sunCoords.declination) * cos(moonCoords.declination) -
            cos(sunCoords.declination) *
                sin(moonCoords.declination) *
                cos(sunCoords.rightAscension - moonCoords.rightAscension)
    );
